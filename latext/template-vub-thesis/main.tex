...
\subsection{Deploying a reactor}
...
The Haai virtual machine needs the following arguments to start: 
\begin{description}
	\item[reactor bytecode] Bytecode obtained from the compilation of the reactor with the Haai compiler.
	\item[connect source x] A function that the Hvm can use to connect input to the sources of the reactor. At the start of the next iteration this function will be called to update the sources of the deployed reactor.
	\item[handle sink] the function that after each iteration is called to handle the new sinks rpoduced by the deployed reactor.
\end{description}
...
No lifting, so we need these reactors cannot perform side effects..
to perform IO there are special reactors outside of the program (2.5 in witouth funcitons)
